...
long FAR PASCAL WndProc(HWND hWnd, UINT wMsg, WPARAM wParam, LPARAM lParam)
{
   ...
   long returnValue;
   if(theMouseManager->MessageNotify(hWnd, wMsg, wParam, lParam, &returnValue))
      return returnValue;
   theKeyboardManager->MessageNotify(hWnd, wMsg, wParam, lParam);
   if(theScreen->MessageNotify(hWnd, wMsg, wParam, lParam, &returnValue))
      return returnValue;
